...
### 선행 스킬 (입력 생성)
- `gil-content:blog` — 블로그 포스트 생성
- `gil-content:landing-page` — 랜딩 페이지 카피라이팅
- `gil-content:newsletter` — 뉴스레터 작성
- `gil-content:copywriting` — 마케팅 카피 작성
- `gil-office:docx-generator` — DOCX 문서 생성
- `gil-office:pptx-designer` — PPTX 발표자료 생성

### 후속 스킬 (선택적)
- `gil-media:nano-banana` — (선택) 블로그 발행 체인의 이미지 생성 단계
