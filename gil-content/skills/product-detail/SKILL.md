...
description: >
  ...
  페어 gil-commerce:detail-page-copy와 명확히 구분 — 본 스킬은 코드 산출, 페어는 13섹션 카피 JSON 산출.
  체이닝 권장: detail-page-planner(기획·Brief) → detail-page-copy(카피) → product-detail(코드) → detail-page-image(이미지). 페어 gil-content:landing-page와 구분 — 본 스킬은 카탈로그 상세, landing-page는 단독 전환 1페이지.
