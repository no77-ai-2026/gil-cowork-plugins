...
description: >
  [한·UZ 듀얼] [책임 경계] 한국 이커머스 상세페이지의 **기획·구조·전략 설계**.
...
| 단계 | 스킬 | 산출물 |
|------|------|--------|
| 1. 기획·전략 (★ **본 스킬**) | `gil-content:detail-page-planner` | **Brief** (원 메시지·오프닝·본문 뼈대·채널·이미지 체크리스트) |
| 2. 카피 | `gil-commerce:detail-page-copy` | 13섹션 감정여정 JSON |
| 3. 코드 | `gil-content:product-detail` | shadcn/ui React 또는 HTML |
| 4. 이미지 | `gil-commerce:detail-page-image` | 1080×12720 합성 PNG |
...
후속 스킬 호출:
```
1. brief 산출 후 → gil-commerce:detail-page-copy "Brief: <yaml>"
2. 카피 JSON 산출 후 → gil-content:product-detail "Brief + Copy"
3. 코드 산출 후 → gil-commerce:detail-page-image "Brief + Copy + Code"
```
...
- "상세페이지 카피 만들어줘" → `gil-commerce:detail-page-copy` (13섹션 JSON)
- "상세페이지 코드 만들어줘" → `gil-content:product-detail` (shadcn/ui 컴포넌트)
- "상세페이지 이미지 합성해줘" → `gil-commerce:detail-page-image` (1080×12720 PNG)
- "랜딩 페이지 만들어줘" → `gil-content:landing-page` (단독 전환 1페이지)
...
- alternative: 빠른 단일 카피만 → gil-commerce:detail-page-copy 직접
- 가이드: `gil-foundation-cc` (Claude Code 스킬 작성), `.claude/rules/moai/development/skill-authoring.md`
