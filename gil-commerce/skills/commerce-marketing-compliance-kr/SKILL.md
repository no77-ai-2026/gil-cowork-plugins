...
description: |
  [한·UZ 듀얼] [책임 경계] 한국 정보통신망법(정통망법) 광고·정보성 메시지 자동 판정 + 야간 발송 차단 + 표기 위치 검증 + 무료 수신거부 명시 점검 전담. 모든 채널(SMS/LMS/MMS·이메일·앱푸시·카톡 친구톡/알림톡·텔레마케팅) 발송 전 의무 통과 법규 게이트. 페어 스킬 commerce-channel-message(메시지 카피 작성)와 명확히 구분 — 본 스킬은 발송 전 사전 점검, 페어는 카피 생성.
...
`.gil/specs/SPEC-COMPLIANCE-KR-001/spec.md` (v2.6.0 신규)
